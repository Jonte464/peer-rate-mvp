...
// Visar endast knapp om:
// 1) sidan ser ut som en relevant ordervy
// 2) vi kan extrahera tillräcklig deal-data
// 3) backend INTE redan säger att affären är rated
...
(function () {
  ...
  async function evaluatePage() {
    ...
    try {
      ...
      if (latestPayload) {
        injectOrUpdateButton(latestPayload);
      }
    } finally {
      isEvaluating = false;
    }
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (!result?.ok && result?.opened !== true) {
            alert('PeerRate: Kunde inte öppna betygsflödet just nu. Försök igen.');
          }
        } catch (err) {
          console.warn('[PeerRate extension] click handler failed:', err);
          alert('PeerRate: Tekniskt fel. Försök igen.');
        } finally {
          btn.disabled = false;
